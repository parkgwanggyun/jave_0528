...
public class PhoneEx01 {
	...
	/**
	 * 기능 : 연락처리스트 에서 이름에 맞는 연락처를 출력하는 메소드
	 * @param list 연락처 리스트
	 * @param count 저장된 연락처 개수
	 * @param name 검색할 이름
	 ...
	 * @param name 확인하려는 이름
	 * @param index 번지
	 * @return 선택한 번지에 이름이 주어진 이름과 같은지 다른지를 
	 */
	...
	/**
	 * 기능 : 연락처리스트에서 연락처 입력받아를 수정하는 메소드
	 * @param list 연착처 리스트
	 * @param count 저장된 개수
	 * @throws Exception 
	 */
...
class Contact{  //시작
	...
	//hashCode equals 는 전화번호가 같으면 등록이 되면 안되고 , 수정할 때도
	//있는 번호는 등록되면 안되기 때문에 equals를 오버라이딩 하면 같은 번호인지
	//아닌지 비교하기가 쉽다.
	@Override
	public int hashCode() {
	...
	//toString을 오버라이딩하면 이름 : 번호 형태의 문자열이 필요할 때 사용하기가
	//좋다(여러번 필요하기 때문에)
	@Override
