...
public class StudentManager implements Program{
	...
	private void studentInfoUpdate() {
		...
		//번지가 유효하지 않은 번지이면 => 번지가 0보다 작으면 알림문구 출력후 종료
		if(index < 0) {
		...
		//update메소드는 Student 클래스에서 새로 추가해야함
	...
	private Subject inputSubject() {
		...
		int prefomace = scan.nextInt();
		subject.update(midterm,finals,prefomace);
		return subject;
	}


	private void updateSubjetScore() {
		//학생 정보를 입력하여 객체를 생성
		Student std = inputStudent();
	...
	private void deleteSubjectScore() {
		...
		//과목이 과목리스트에 없으면 안내문구 출력 후 종료
		if(!subjectList.contains(subject.getName())) {
			System.out.println("등록되지 않은 과목성적이어서 삭제할 수 없습니다.");
			return;
		}
		//학생 과목 리스트에서 과목 객체를 삭제하여 성공하면 안내문구 출력후 종료
		if(subjects.remove(subject)) {
			System.out.println("과목 성적을 삭제했습니다.");
			return;
		}
		//실패하면 안내문구 출력 후 종료
		System.out.println("삭제의 실패했습니다.");
		
	}
	private Subject inputRequiredSubject() {
		scan.nextLine();
	...
	private void updateSubject() {
		...
		//수정할 과목이 있느지 없는지 확인해서 없으면 알림 문구 출력후 종료
		...
		//새 과목명이 이미 등록되어 있으면 안내 문구 출력 후 종료
	...
	private void insertSubject() {
		...
		if(subjectList.contains(sub)) {
			System.out.println("이미등록된 과목입니다.");
			return;
		}
		//과목 리스트에 과목을 추가
		subjectList.add(sub);
